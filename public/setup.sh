#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════
#   🤖 اسکریپت راه‌انداز «نقلی‌استودیو»
#   ربات خودکار تولید و انتشار ویدئوی کودک در تیک‌تاک
#
#   سیستم‌عامل: Ubuntu 20.04+ / Debian 11+
#
#   استفاده:
#     sudo bash setup.sh                        ← نصب کامل
#     sudo bash setup.sh --domain example.com   ← نصب + دامنه و SSL خودکار
...
db_url()   { echo "postgresql://${DB_USER}:${DB_PASS}@127.0.0.1:5432/${DB_NAME}"; }

echo ""
echo "════════════════════════════════════════════"
echo "   🤖 راه‌انداز نقلی‌استودیو — نسخه ۲.۴"
echo "════════════════════════════════════════════"
echo ""
...
if [[ "$ACTION" == "update" ]]; then
  ...
  cd "$APP_DIR"
  if [[ -d .git ]]; then log "دریافت آخرین تغییرات از گیت…"; git pull --ff-only || warn "git pull ناموفق بود؛ از کد فعلی استفاده می‌شود."; fi
  log "نصب وابستگی‌ها…"
...
if [[ "$(realpath "$SRC_DIR")" != "$(realpath "$APP_DIR" 2>/dev/null || echo /nonexistent)" ]]; then
...
Documentation=https://your-domain/setup
...
if systemctl is-active --quiet "${SERVICE_NAME}"; then
  ...
  warn "سرویس هنوز فعال نشده — لاگ‌ها را ببینید: journalctl -u ${SERVICE_NAME} -n 50"
fi
...
EOF
  ...
  WEBHOOK_HINT="http://YOUR_SERVER_IP:${APP_PORT}/api/webhook (برای تلگرام به HTTPS و دامنه نیاز دارید)"
fi
...
if command -v ufw >/dev/null 2>&1 && ufw status | grep -q "active"; then
  ...
  [[ -z "$DOMAIN" ]] && ufw allow "${APP_PORT}/tcp" >/dev/null 2>&1 || true
...
echo "      ${DB_PASS}"
echo ""
echo "   📡 آدرس وب‌هوک تلگرام (در تنظیمات ربات وارد کنید):"
echo "      ${WEBHOOK_HINT}"
echo ""
...
echo "      sudo bash setup.sh --update               ← به‌روزرسانی"
echo "      sudo bash setup.sh --status               ← گزارش سریع"
echo ""
